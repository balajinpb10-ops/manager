...
class CryptoManager(private val context: Context) {
    ...
    /** After restoring from a backup, the backup's password becomes this device's new master
     *  password (matching what the restore UI tells the user). A previous version of this
     *  function stored the ORIGINAL device's verifier (recovered from the backup) alongside a
     *  freshly-derived key for the backup password — since those came from two different
     *  passwords, the very next unlock attempt with the "correct" (backup) password would fail
     *  verification. It now behaves like changeMasterPassword: derive a brand new salt/verifier
     *  pair for the password actually being used from here on. */
    fun reKeyFromRestore(password: CharArray): ByteArray = changeMasterPassword(password)
